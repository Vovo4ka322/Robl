...
namespace Codebase.Game.Gameplay.Player
{
    public class PlayerController : MonoBehaviour
    {
        ...
        [SerializeField] private LayerMask _trampolineLayers;
        ...
        private float _jumpForceOnTrampoline = 25;

        private bool IsGrounded => Physics.Raycast(transform.position, Vector3.down, 0.2f, _groundLayers);

        private bool IsTrampolined => Physics.Raycast(transform.position, Vector3.down, 0.2f, _trampolineLayers);
        ...
        private void Jump()
        {
            if (IsTrampolined)
            {
                _rigidBody.velocity = new Vector3(_rigidBody.velocity.x, _jumpForceOnTrampoline, _rigidBody.velocity.z);
                _animator.SetBool("IsTrampolined", IsTrampolined);
            }
        ...
        {
            _animator.SetBool("IsMeditated", true);
        ...
        {
            _animator.SetBool("IsMeditated", false);
        }
    }
}
